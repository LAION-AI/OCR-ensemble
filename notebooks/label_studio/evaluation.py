...
# get string from either a pikle path or a json path 
def get_string(str_path, str_key=""):
    result = ""
    if isinstance(str_path, list): # is alread a list of strings
        result = str_path
    elif str_path.endswith('.json'): # we have a json file
        with open(str_path, 'r') as file:
            result = json.load(file)
        if isinstance(result, dict):
            result = result[str_key]
    else: # it is a .pkl file
        with open(str_path, 'rb') as f:
            result = pickle.load(f)
    return result


def scores(val_path, pred_path, val_key="", pred_key=""):
    values = get_string(val_path, val_key)
    preds = get_string(pred_path, pred_key)
    print(f"Prediciton: {preds}")
    print(f"Values: {values}")
    scorers = [CERScorer(None), WPAScorer(None), AccEDScorer(None), SROIEScorer(None)]
    for val, pred in zip(values, preds):
        for scorer in scorers:
            scorer.add_string(val, pred)
    scores = {}
    scores["CER"] = scorers[0].score()
    scores["WPA"] = scorers[1].score()
    scores["Accuracy"], scores["Norm ED"] = scorers[2].score()
    scores["Precesion"], scores["Recall"], scores["F1"] = scorers[3].score()
    return scores
